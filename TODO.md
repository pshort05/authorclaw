...
- [ ] **`gateway/src/index.ts` is ~2,650 lines and owns ~50 service instances.** Initialization is one numbered Phase sequence. Worth extracting the per-phase init into separate modules so the entry point becomes a thin composition root rather than a god-class. Not a refactor to start without an explicit goal — note for the next time index.ts blocks a feature.
- [ ] **`gateway/src/api/routes.ts` is ~5,500 lines.** A single factory mounts every endpoint. Split per feature area (projects, personas, vault, voice, website, wave3) once it next causes a merge conflict or readability issue.
