# OpenClaw → AuthorClaw Feature-Backport Candidates
...
- Local AuthorClaw tree (`/home/paul/data/dev/authorclaw`) — last upstream sync was commit `80df746` *"Reasoning + browser pieces from OpenClaw 2026.4.24/25"*, so anything below has shipped upstream **after** AuthorClaw's last pull.
...
**Author payoff:** Dictate prose hands-free on a walk. Voice brainstorming during a stuck scene. Read a chapter aloud and have the agent suggest line edits in conversation. AuthorClaw currently has *output-only* TTS (9 Edge presets) and no voice **input** path at all.
...
**Why this rank:** Nonfiction authors and ghostwriters live in transcripts. Without a real transcripts pipeline, AuthorClaw can't serve memoir / business-book / podcast-to-book customers.
...
**Author payoff:** Visualize the series bible — character relationship graphs, beat-sheet boards, plot-promise heatmaps, timeline strips, world maps. AuthorClaw's planning artifacts (`story-structures`, `plot-promises`, `series-bible`) are currently *.md only — no visual layer.
...
**Author payoff:** Real comp-title research on Amazon/Goodreads/BookBub. KDP dashboard scraping for sales/royalty data. Reading reviews of competing books for blurb angles. AuthorClaw's current `ResearchGate` is HTTP-fetch + HTML extraction with a domain allowlist — fine for Wikipedia, useless for JS-rendered sites.
...
**Author payoff:** Today AuthorClaw memory is SQLite FTS5 — keyword search. Embeddings turn the series bible + all prior chapters + character voice samples into a *semantic* index. "Find every scene where Riley feels watched" stops requiring the exact word "watched." Critical at the 50k-word mark of a draft when the model can't hold the whole manuscript in context.
...
**Author payoff:** Capture story ideas in the field. Photo a bookstore display → research the genre's current cover trends. Voice-note a scene fragment while driving. Inhale a competitor's blurb via screen capture. Today, AuthorClaw is reachable only via Telegram, Discord, or the LAN dashboard.
...
**Author payoff:** Most non-technical author clients live in WhatsApp/iMessage, not Telegram. Reaction approvals (👍 on a draft = "approved, continue") fit the confirmation-gate pattern AuthorClaw already enforces, and they replace typing `yes` with a tap. Drops the friction of the confirmation gate by an order of magnitude on phone.
...
**Author payoff:** Different pen names with different billing accounts (a Lily Hart Anthropic key vs. a TD Wood Anthropic key, so costs are attributable per persona). Today AuthorClaw's vault stores one key per provider, full stop.
...
**Author payoff:** Book trailers. Reels/Shorts/TikTok marketing assets. A scene's "moodboard" rendered as a 5-second clip for the author's own visualization. AuthorClaw already has Gemini Nano Banana for stills; video closes the loop.
**Effort:** Low if AuthorClaw's router has a clean provider-plugin interface. Medium if it doesn't.
...
**Author payoff:** Today AuthorClaw skills live in `skills/{author,core,marketing,ops,premium}/` and ship in-tree. A managed-skill story would let third-party authors publish a "Regency Romance Bible" skill or a "Brandon-Sanderson-style magic-system" skill that anyone can install. Already half-aligned with the `auto-skill` service.
...
- **No-auth Tailscale exposure rejection** — relevant if AuthorClaw ever follows OpenClaw's Tailscale path.
- **Admin requirement for node/device-role approvals** — needed once #6 lands.

**Why this rank:** The standing AuthorClaw TODO already lists "Full security review (post-LAN-exposure)" as the next big rock. Most of these defenses map 1:1 to items on that list. Cheap to port the ones that apply, expensive to design from scratch later.
...
**Author payoff:** AuthorClaw's `gateway/src/index.ts` is 2,650 lines that runs a numbered initialization phase sequence. Cold-start is slow. These caches address the same pattern.
...
**Author payoff:** AuthorClaw runs in Docker today. Adding an SSH backend would let a single Mercury install drive jobs on Neptune (the n8n + rclone host) for big batch work without copying state. Aligns with the existing Mercury↔Neptune NFS architecture.
...
**Author payoff:** The `heartbeat` and `release-calendar` services schedule recurring jobs through AuthorClaw's `cron-scheduler`. Concurrency 1 (or whatever the current default is) bottlenecks the heartbeat. Windows backend matters if any author runs the agent on a Windows box.
...
**Author payoff:** Sharp's native binary is the single most painful dependency for cross-platform installs (the AuthorClaw README's "fail-soft init" pattern exists partly because of this class of problem). Removing it simplifies the Docker image and unblocks Windows-native installs.
...
Useful if the agent author is on macOS, but adds packaging/signing infrastructure cost (Apple Developer account, notarization) that hits AuthorClaw the moment it ships an installable. Park until #6 is committed to.
...
Mostly relevant if AuthorClaw ever splits compute across hosts (Mercury + Neptune). #16 (SSH sandbox) probably gets to the same outcome with less infrastructure.
...
Internal release-engineering improvements. Pick up if/when AuthorClaw stands up its own release pipeline. Useful as a reference *list* even if not ported directly.
